...
?>
<h3 style="text-transform: uppercase;text-align:center;">SCHEDULE SHOWING THE RECOVERY OF CYCLE ADVANCE INTEREST IN R/O <?php echo ($model->BILL_TYPE == 1)? "OPS":"NPS"; ?> STAFF OF  <?php echo $master->DEPT_NAME?>FOR THE MONTH OF <?php echo date('M-Y', strtotime($model->CREATION_DATE))?></h3>
...
<table class="pay-schedule-table small">
	...
	<?php 
		...
		$criteria->condition = 't.BILL_ID_FK='.$model->ID.' AND t.CYCLE_EMI > 0 AND t.IS_CYCLE_RECOVERY = 1';
	...
	<tfoot>
		...
		<th class="small right-br"></th>
		<?php } ?>
		<th class="small-xx"><?php $CYCLE_EMI = Yii::app()->db->createCommand("SELECT SUM(CYCLE_EMI) as CYCLE_EMI FROM tbl_salary_details WHERE BILL_ID_FK = $model->ID AND IS_CYCLE_RECOVERY=1;")->queryRow()['CYCLE_EMI']; echo $CYCLE_EMI;?></th>
		<th class="small-xx"><?php $CYCLE_TOTAL = Yii::app()->db->createCommand("SELECT SUM(CYCLE_TOTAL) as CYCLE_TOTAL FROM tbl_salary_details WHERE BILL_ID_FK = $model->ID AND IS_CYCLE_RECOVERY=1;")->queryRow()['CYCLE_TOTAL']; echo $CYCLE_TOTAL;?></th>
		<th class="small-xx"></th>
		<th class="small-xx"><?php $CYCLE_BAL = Yii::app()->db->createCommand("SELECT SUM(CYCLE_BAL) as CYCLE_BAL FROM tbl_salary_details WHERE BILL_ID_FK = $model->ID AND IS_CYCLE_RECOVERY=1;")->queryRow()['CYCLE_BAL']; echo $CYCLE_BAL; ?></th>
	</tfoot>
</table>
