...
?>
<h3 style="text-transform: uppercase;text-align:center;">SCHEDULE SHOWING THE RECOVERY OF COURT ATTACHMENT IN R/O <?php echo ($model->BILL_TYPE == 1)? "OPS":"NPS"; ?> STAFF OF  <?php echo $master->DEPT_NAME?> FOR THE MONTH OF <?php echo date('M-Y', strtotime($model->CREATION_DATE))?></h3>
...
<table class="pay-schedule-table small">
	<thead>
		<tr>
			...
			<?php } ?>
			<th class="small-xx">COURT ATTACHMENT </th>
		</tr>
	</thead>
	<?php 
		$i = 1;	
		...
		$criteria->condition = 't.BILL_ID_FK='.$model->ID.' AND t.COURT_ATTACHMENT > 0';
						...
						if($j==1){
							?>
								<tr>
									...
									<td class="small right-br"><?php echo $period['FORMAT']; ?></td>
									<td class="small-xx"><?php echo $salary->COURT_ATTACHMENT; ?></td>
								</tr>
							<?php
						}
						else{
	...
	<tfoot>
		...
		<th class="small right-br"></th>
		<?php } ?>
		<th class="small-xx"><?php $COURT_ATTACHMENT = Yii::app()->db->createCommand("SELECT SUM(COURT_ATTACHMENT) as COURT_ATTACHMENT FROM tbl_salary_details WHERE BILL_ID_FK = $model->ID AND YEAR = $model->YEAR AND MONTH = $model->MONTH;")->queryRow()['COURT_ATTACHMENT']; echo $COURT_ATTACHMENT;?></th>
	</tfoot>
</table>

<p style="text-align: center; margin-top: 30px;"><b><?php echo $this->amountToWord($COURT_ATTACHMENT);?><b></p>
