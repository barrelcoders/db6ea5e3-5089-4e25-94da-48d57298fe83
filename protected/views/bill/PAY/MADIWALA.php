...
?>
<h3 style="text-transform: uppercase;text-align:center;">SCHEDULE SHOWING THE RECOVERY OF MADIWALA MAINTENANCE IN R/O <?php echo ($model->BILL_TYPE == 1)? "OPS":"NPS"; ?> STAFF OF  <?php echo $master->DEPT_NAME?>FOR THE MONTH OF <?php echo date('M-Y', strtotime($model->CREATION_DATE))?></h3>
...
<table class="pay-schedule-table small">
	<thead>
		<tr>
			...
			<?php } ?>
			<th class="small-xx">MADIWALA MAINTENANCE </th>
		</tr>
	</thead>
	<?php 
		$i = 1;	
		...
		$criteria->condition = 't.BILL_ID_FK='.$model->ID.' AND t.MAINT_MADIWALA > 0';
						...
						if($j==1){
							?>
								<tr>
									...
									<td class="small-xx"><?php echo $salary->MAINT_MADIWALA; ?></td>
								</tr>
							<?php
						}
						else{
	...
	<tfoot>
		...
		<th class="small right-br"></th>
		<?php } ?>
		<th class="small-xx"><?php $MAINT_MADIWALA = Yii::app()->db->createCommand("SELECT SUM(MAINT_MADIWALA) as MAINT_MADIWALA FROM tbl_salary_details WHERE BILL_ID_FK = $model->ID AND YEAR = $model->YEAR AND MONTH = $model->MONTH;")->queryRow()['MAINT_MADIWALA']; echo $MAINT_MADIWALA;?></th>
	</tfoot>
</table>

<p style="text-align: center; margin-top: 30px;"><b><?php echo $this->amountToWord($MAINT_MADIWALA);?><b></p>
