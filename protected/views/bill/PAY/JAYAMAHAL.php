...
?>
<h3 style="text-transform: uppercase;text-align:center;">SCHEDULE SHOWING THE RECOVERY OF JAYAMAHAL MAINTENANCE IN R/O <?php echo ($model->BILL_TYPE == 1)? "OPS":"NPS"; ?> STAFF OF  <?php echo $master->DEPT_NAME?>FOR THE MONTH OF <?php echo date('M-Y', strtotime($model->CREATION_DATE))?></h3>
...
<table class="pay-schedule-table small">
	<thead>
		<tr>
			...
			<?php } ?>
			<th class="small-xx">JAYAMAHAL MAINTENANCE</th>
		</tr>
	</thead>
	<?php 
		$i = 1;	
		...
		$criteria->condition = 't.BILL_ID_FK='.$model->ID.' AND t.MAINT_JAYAMAHAL > 0';
						...
						if($j==1){
							?>
								<tr>
									...
									<td class="small-xx"><?php echo $salary->MAINT_JAYAMAHAL; ?></td>
								</tr>
							<?php
						}
						else{
	...
	<tfoot>
		...
		<th class="small right-br"></th>
		<?php } ?>
		<th class="small-xx"><?php $MAINT_JAYAMAHAL = Yii::app()->db->createCommand("SELECT SUM(MAINT_JAYAMAHAL) as MAINT_JAYAMAHAL FROM tbl_salary_details WHERE BILL_ID_FK = $model->ID AND YEAR = $model->YEAR AND MONTH = $model->MONTH;")->queryRow()['MAINT_JAYAMAHAL']; echo $MAINT_JAYAMAHAL;?></th>
	</tfoot>
</table>

<p style="text-align: center; margin-top: 30px;"><b><?php echo $this->amountToWord($MAINT_JAYAMAHAL);?><b></p>
