...
<h3 style="text-transform: uppercase;text-align:center;">SCHEDULE SHOWING THE RECOVERY OF COMPUTER ADVANCE IN R/O <?php echo ($model->BILL_TYPE == 1)? "OPS":"NPS"; ?> STAFF OF  <?php echo $master->DEPT_NAME?>FOR THE MONTH OF <?php echo date('M-Y', strtotime($model->CREATION_DATE))?></h3>
...
<table class="pay-schedule-table small">
	...
	<?php 
		...
		$criteria->condition = "BILL_ID_FK=$model->ID AND COMP_EMI > 0 AND IS_COMP_RECOVERY = 0";
	...
	<tfoot>
		...
		<th class="small right-br"></th>
		<th class="small-xx"><?php $COMP_EMI = Yii::app()->db->createCommand("SELECT SUM(COMP_EMI) as COMP_EMI FROM tbl_salary_details WHERE BILL_ID_FK = $model->ID AND IS_COMP_RECOVERY = 0;")->queryRow()['COMP_EMI']; echo $COMP_EMI;?></th>
		<th class="small-xx"><?php $COMP_TOTAL = Yii::app()->db->createCommand("SELECT SUM(COMP_TOTAL) as COMP_TOTAL FROM tbl_salary_details WHERE BILL_ID_FK = $model->ID AND IS_COMP_RECOVERY = 0;")->queryRow()['COMP_TOTAL']; echo $COMP_TOTAL;?></th>
		<th class="small-xx"></th>
		<th class="small-xx"><?php $COMP_BAL = Yii::app()->db->createCommand("SELECT SUM(COMP_BAL) as COMP_BAL FROM tbl_salary_details WHERE BILL_ID_FK = $model->ID AND IS_COMP_RECOVERY = 0;")->queryRow()['COMP_BAL']; echo $COMP_BAL; ?></th>
	</tfoot>
</table>
