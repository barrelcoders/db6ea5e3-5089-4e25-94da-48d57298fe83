...
?>
<h3 style="text-transform: uppercase;text-align:center;">SCHEDULE SHOWING THE RECOVERY OF LF IN R/O <?php echo ($model->BILL_TYPE == 1)? "OPS":"NPS"; ?> STAFF OF  <?php echo $master->DEPT_NAME?>FOR THE MONTH OF <?php echo date('M-Y', strtotime($model->CREATION_DATE))?></h3>
...
<table class="pay-schedule-table small">
	...
	<?php 
		...
		$criteria->condition = 't.BILL_ID_FK='.$model->ID.' AND t.LF > 0';
						...
						if($j==1){
							?>
								<tr>
									...
									<td class="small-xx"><?php echo $salary->LF; ?></td>
								</tr>
							<?php
						}
						else {
...
<p style="text-align: center; margin-top: 50px;"><b><?php echo $this->amountToWord($LF);?><b></p>
