...
<h3 style="text-transform: uppercase;text-align:center;">SCHEDULE SHOWING THE RECOVERY OF ASSOSCIATION SUBSCRIPTION IN R/O <?php echo ($model->BILL_TYPE == 1)? "OPS":"NPS"; ?> STAFF OF  <?php echo $master->DEPT_NAME?>FOR THE MONTH OF <?php echo date('M-Y', strtotime($model->CREATION_DATE))?></h3>
...
<table class="pay-schedule-table small">
	...
	<?php 
		...
		$criteria->condition = "BILL_ID_FK=$model->ID AND ASSOSC_SUB > 0";
	...
	<tfoot>
		...
		<th class="small right-br"></th>
		<th class="small-xx"><?php $ASSOSC_SUB = Yii::app()->db->createCommand("SELECT SUM(ASSOSC_SUB) as ASSOSC_SUB FROM tbl_salary_details WHERE BILL_ID_FK = $model->ID AND YEAR = $model->YEAR AND MONTH = $model->MONTH;")->queryRow()['ASSOSC_SUB']; echo $ASSOSC_SUB;?></th>
	</tfoot>
</table>

<p style="text-align: center; margin-top: 30px;"><b><?php echo $this->amountToWord($ASSOSC_SUB);?><b></p>
