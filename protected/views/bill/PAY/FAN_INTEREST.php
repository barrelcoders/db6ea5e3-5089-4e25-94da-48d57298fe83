...
<h3 style="text-transform: uppercase;text-align:center;">SCHEDULE SHOWING THE RECOVERY OF FAN INTEREST IN R/O <?php echo ($model->BILL_TYPE == 1)? "OPS":"NPS"; ?> STAFF OF  <?php echo $master->DEPT_NAME?>FOR THE MONTH OF <?php echo date('M-Y', strtotime($model->CREATION_DATE))?></h3>
...
<table class="pay-schedule-table small">
	...
	<?php 
		...
		$criteria->condition = "BILL_ID_FK=$model->ID AND FAN_EMI > 0 AND IS_FAN_RECOVERY = 1";
	...
	<tfoot>
		...
		<th class="small right-br"></th>
		<th class="small-xx"><?php $FAN_EMI = Yii::app()->db->createCommand("SELECT SUM(FAN_EMI) as FAN_EMI FROM tbl_salary_details WHERE BILL_ID_FK = $model->ID AND IS_FAN_RECOVERY=1;")->queryRow()['FAN_EMI']; echo $FAN_EMI;?></th>
		<th class="small-xx"><?php $FAN_TOTAL = Yii::app()->db->createCommand("SELECT SUM(FAN_TOTAL) as FAN_TOTAL FROM tbl_salary_details WHERE BILL_ID_FK = $model->ID AND IS_FAN_RECOVERY=1;")->queryRow()['FAN_TOTAL']; echo $FAN_TOTAL;?></th>
		<th class="small-xx"></th>
		<th class="small-xx"><?php $FAN_BAL = Yii::app()->db->createCommand("SELECT SUM(FAN_BAL) as FAN_BAL FROM tbl_salary_details WHERE BILL_ID_FK = $model->ID AND IS_FAN_RECOVERY=1;")->queryRow()['FAN_BAL']; echo $FAN_BAL; ?></th>
	</tfoot>
</table>
